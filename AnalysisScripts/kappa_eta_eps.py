...
def estimate_epsilon_br_ns(root_dir, in_dim, out_dim):
    ...
    generations_to_use = range(
        0, 2000, 10)  #we skip some otherwise pdist is intractable
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Compute Kappa and Eta metrics from the Q-matrix')
    parser.add_argument(
        '--qmat',
        help=
        'the Q matrix defined in the BR-NS paper, which can be computed from the tools in cycling_analysis.py',
        required=True)
    parser.add_argument('--problem_type',
                        help='can be either \"archive_based\" or \"br-ns\"',
                        required=True)
    parser.add_argument(
        '--env',
        help='can be either \"large_ant\" or \"deceptive_maze\"',
        required=True)
    parser.add_argument(
        '--epsilon',
        help=
        ' The epsilon from the BR-NS paper. If not specified, computed automatically (in the case of br-ns, you will also need to set root_dir to compute epsilon)',
        required=False,
        type=float)
    parser.add_argument(
        '--root_dir',
        help=
        'If you want epsilon to be computed for BR-NS, you will need to set this parameter to a directory which contains frozen_net.model',
        required=False)
    parser.add_argument(
        '--skipped_qm',
        help=
        'As computing the Q matrix is very time-consuming (especially for large archive sizes, sometimes Q(i,j) is only computed at regular intervales where skipped_qm generations are skipped, if so, you should indicate that here.',
        required=False,
        default=1,
        type=int)
    ...
    if args.epsilon is None:
        ...
        if args.problem_type == "archive_based":
            ...
            if args.env == "large_ant":
                ...
                eps = estimate_epsilon_archive_based(
                    np.array([[0, 600], [0, 600]]))
            else:
            ...
            if args.root_dir is None:
                raise Exception(
                    "You didn't specifiy epsilon for BR-NS so root_dir is required to compute it"
                )
    ...
    if RANDOM:
        ...
        for qm_i in range(N):
            ...
            plt.plot(qm[i, :],
                     label=f"population at generation {i}",
                     linewidth=5)
        ...
        for i in lst:
            if args.skipped_qm is not None:
                ...
                scaled_range = [
                    x * args.skipped_qm for x in range(qm.shape[0])
                ]
            plt.plot(scaled_range,
                     qm[idx, :],
                     label=f"population at generation {i}",
                     linewidth=5)
